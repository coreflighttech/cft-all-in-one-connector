...
namespace MobiFlight
{
    ...
    {
        private static bool IsDedicatedDevice(MobiFlightModule module)
        {
            return module != null
                && !String.IsNullOrWhiteSpace(module.Name)
                && (module.Name.StartsWith("CoreFlightTech", StringComparison.OrdinalIgnoreCase)
                    || module.Name.StartsWith("Core Flight Tech", StringComparison.OrdinalIgnoreCase)
                    || module.Name.StartsWith("CFT_", StringComparison.OrdinalIgnoreCase));
        }
        /// <summary>
...
#pragma warning restore IDE0044 // Add readonly modifier
        ...
        private async void SerialPortMonitor_PortAvailable(object sender, PortDetails portDetails)
        {
            ...
            // This fork must never retain or execute another MobiFlight device.
            // The name is read from the module firmware, not inferred from COM number.
            if (!IsDedicatedDevice(result))
            {
                Log.Instance.log($"Dedicated mode: ignoring module '{result?.Name}' on {portDetails.Name}.", LogSeverity.Info);
                result?.Disconnect();
                await CheckIfLookUpFinished();
                return;
            }

            // Keep the real firmware serial intact. MobiFlight's native
            // ControllerBindingService.PerformAutoBinding flow performs the binding.
            Log.Instance.log($"Dedicated mode: accepted '{result.Name}' ({result.Serial}) on {portDetails.Name}; native auto-binding remains enabled.", LogSeverity.Info);
        ...
        private async void UsbDeviceMonitor_PortAvailable(object sender, PortDetails e)
        {
            ...
            Log.Instance.log($"Dedicated mode: ignoring USB device '{e.Name}'.", LogSeverity.Info);
            await CheckIfLookUpFinished();
            return;
