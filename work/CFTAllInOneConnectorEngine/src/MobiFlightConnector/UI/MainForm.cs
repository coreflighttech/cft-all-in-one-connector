...
namespace MobiFlight.UI
{
    public partial class MainForm : Form, IProjectToolbar, INotifyPropertyChanged
    {
        ...
        private bool coreFlightRunRequested = false;
        private readonly bool coreFlightHiddenMode = Environment.GetCommandLineArgs()
            .Any(arg => arg.Equals("/coreFlightHidden", StringComparison.OrdinalIgnoreCase));
        ...
        public MainForm()
        {
            ...
            // The dedicated CoreFlightTech appliance is controlled by the compact
            // launcher. Keep the inherited MobiFlight window fully invisible while
            // preserving its message loop and native execution engine.
            if (coreFlightHiddenMode)
            {
                ShowInTaskbar = false;
                Opacity = 0;
        ...
        private async void OnFrontendReady(object sender, EventArgs e)
        {
            ...
            // Dedicated appliance mode: never block startup with welcome, release,
            // donation, or onboarding dialogs. The controller must start unattended.
        ...
        void ExecManager_OnInitialModuleLookupFinished(object sender, EventArgs e)
        {
            ...
            Log.Instance.log("CoreFlightTech READY: native device lookup and profile loading completed.", LogSeverity.Info);

            // Now we are done with all initialization stuff
            // and now we are ready to look for sims
            execManager.AutoConnectStart();
            TryStartCoreFlightExecution();
        ...
        private void Module_Connected(object sender, EventArgs e)
        {
            ...
            // If the dedicated device is attached after the initial lookup, the
            // project was loaded while no matching module existed. Reload through
            // MobiFlight's native config path so its normal auto-binding runs for
            // the newly connected module, then resume Run when CONNECT is active.
            var moduleInfo = module.ToMobiFlightModuleInfo();
            if (coreFlightHiddenMode &&
                (moduleInfo?.Name?.StartsWith("CoreFlightTech", StringComparison.OrdinalIgnoreCase) == true ||
                 moduleInfo?.Name?.StartsWith("Core Flight Tech", StringComparison.OrdinalIgnoreCase) == true ||
                 moduleInfo?.Name?.StartsWith("CFT_", StringComparison.OrdinalIgnoreCase) == true) &&
                cmdLineParams?.ConfigFile != null &&
                File.Exists(cmdLineParams.ConfigFile))
            {
                if (execManager.IsStarted())
                    execManager.Stop();

                Log.Instance.log($"CoreFlightTech module connected after startup; reloading profile for native auto-binding: {moduleInfo.Name} ({moduleInfo.Port}).", LogSeverity.Info);
                LoadConfig(cmdLineParams.ConfigFile);
                BeginInvoke(new Action(TryStartCoreFlightExecution));
            }
        ...
        private void _showError(string msg)
        {
            if (coreFlightHiddenMode)
            {
                Log.Instance.log(msg, LogSeverity.Warn);
                return;
            }
        ...
        private void ShowNotification(string eventName, Dictionary<string, string> context, string fallbackMessage)
        {
            if (coreFlightHiddenMode)
            {
                Log.Instance.log(fallbackMessage, LogSeverity.Info);
                return;
            }
        ...
        private void TryStartCoreFlightExecution()
        {
            if (!coreFlightRunRequested || execManager == null || !InitialLookupFinished)
                return;

            if (execManager.IsStarted())
                return;

            Log.Instance.log("CoreFlightTech CONNECT: device lookup and profile loading finished; starting native MobiFlight execution.", LogSeverity.Info);
            StartProjectExecution();
        }
        ...
        {
            const int WM_COREFLIGHTTECH_RUN = 0x8000 + 737;
            const int WM_COREFLIGHTTECH_STOP = 0x8000 + 738;
            const int WM_COREFLIGHTTECH_SHUTDOWN = 0x8000 + 739;

            if (m.Msg == WM_COREFLIGHTTECH_RUN)
            {
                coreFlightRunRequested = true;
                Log.Instance.log("CoreFlightTech CONNECT requested; waiting for native device lookup and profile loading when necessary.", LogSeverity.Info);
                BeginInvoke(new Action(TryStartCoreFlightExecution));
                m.Result = IntPtr.Zero;
                return;
            }
            if (m.Msg == WM_COREFLIGHTTECH_STOP)
            {
                coreFlightRunRequested = false;
                if (execManager != null)
                    BeginInvoke(new Action(StopExecution));
                m.Result = IntPtr.Zero;
                return;
            }
            if (m.Msg == WM_COREFLIGHTTECH_SHUTDOWN)
            {
                coreFlightRunRequested = false;
